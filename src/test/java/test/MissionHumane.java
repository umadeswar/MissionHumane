...
public class MissionHumane {
	...
	public static void writeData(int start, int end) throws IOException {
		...
		for (int i = start; i <= end; i++) {
			...
			data.add("Hospital Name");
			data.add("Without Oxygen");
			data.add("With Oxygen");
			data.add("ICU Without Ventilator");
			data.add("ICU With Ventilator");
			data.add("Last Updated Time");
			data.add("Phone Number");
			data.add("Address");
			xlUtils.writeToExcel(data,"WestBengalSheet",0);
			data.clear();
			...
			//String b = driver.findElement(By.xpath("//table[@class='result-table table table-borderless table-hover']/tbody/tr[" + (i++) + "]/td/div")).getText(); 
			//System.out.println("Col Value 2:" + b);
			String updatedTime = driver.findElement(By.xpath("//table[@class='result-table table table-borderless table-hover']/tbody/tr["+(i++)+"]/td/p[1]"))
					.getText(); 
			System.out.println("Time"+updatedTime);
			data.add(updatedTime);
			String phoneNo = driver.findElement(By.xpath("//table[@class='result-table table table-borderless table-hover']/tbody/tr["+(i) +"]/td/p[2]"))
					.getText(); 
			System.out.println("phone"+phoneNo);
			data.add(phoneNo);
			String address = driver.findElement(By.xpath("//table[@class='result-table table table-borderless table-hover']/tbody/tr["+(i)+"]/td/p[4]")).getText();
			data.add(address);
